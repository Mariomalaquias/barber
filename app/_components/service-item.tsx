...
interface ServiceItemProps {
  ...
  barbershop: Pick<Barbershop, "name">; //dessa forma pego apenas o name que é o unico que vamos utilizar aqui
}
...
const getTimeList = ({ bookings, selectedDay }: GetTimeListprops) => {
  return TIME_LIST.filter((time) => {
    ...
    const hasBookingOnCuurrentTime = bookings.some(
      (booking) =>
        booking.date.getHours() === hour &&
        booking.date.getMinutes() === minutes
    );

    if (hasBookingOnCuurrentTime) {
      return false;
    }
    return true;
  });
                  ...
                  {selectedDay && (
                    <div className="px-5 flex overflow-x-auto p-5 gap-3 border-b border-solid [&::-webkit-scrollbar]:hidden">
                      {timeList.length > 0 ? (
                        ...
                        <p>Não há horarios disponiveis para esse dia.</p>
                      )}
                    </div>
                  )}
